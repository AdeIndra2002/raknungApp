<header class="z-10 py-2 bg-gray-300 dark:bg-gray-800 shadow-md rounded-2xl m-1">
    <div class="container flex justify-between items-center px-6 mx-auto h-full text-purple-600 md:justify-end">
        ...
        <x-dropdown>
            <x-slot name="trigger">
                <button class="text-gray-700 hover:text-gray-500 dark:text-gray-400 hover:dark:text-gray-500 align-middle rounded-full focus:shadow-outline-purple focus:outline-none" @click="toggleProfileMenu" @keydown.escape="closeProfileMenu" aria-label="Account" aria-haspopup="true">
                    {{ Auth::user()->name }}
                </button>
            </x-slot>
        ...
        <!-- Toast Container -->
        <div id="toast-container" class="fixed top-20 right-5 z-50">
    ...
    <script>
        ...
        function showToast(type, message) {
            ...
            var toastClass = type === 'success' ? 'bg-green-500' : 'bg-red-500';
            var toast = `
                <div id="${toastId}" class="toast ${toastClass} text-white p-4 rounded-md shadow-md mb-4">
                    ${message}
                </div>
            `;
        ...
        @if(session('danger'))
        showToast('danger', "{{ session('danger') }}");
        @endif
